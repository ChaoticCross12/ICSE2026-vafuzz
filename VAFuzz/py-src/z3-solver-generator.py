...
def generate_z3_solver(grammar, output_filename):
    """
    Generates a Python file with Z3 declarations and constraints based on the input grammar.
    The output file will be a Z3 solver with a structure that reads additional constraints from a file.
    """
    declarations = []
    constraints = []
    
    def clean_opt_name(name):
        """Helper function to clean option names by only removing leading dashes"""
        if name.startswith('--'):
            return name[2:]
    ...
    def get_prefix_and_name(opt_name):
        """Helper function to extract prefix and clean name"""
    ...
    with open(output_filename, 'w') as solver_file:
        imports = """
import itertools
import random
import string
...
import re
"""
        solver_file.write(imports)
                
        str_to_write = f"""
options = {{
"""
        solver_file.write(str_to_write)
        ...
        for idx, option in enumerate(grammar["options"]):
            prefix, opt_name = get_prefix_and_name(option["opt"])
            opt_type = option["type"]
            prefix_list.append(prefix)
            
            # clean name
            # if '-' in opt_name:
            #     new_opt_name = opt_name.replace("-", "")
                
            #     opt_name_map[new_opt_name] = opt_name
            #     opt_name_revert_map[opt_name] = new_opt_name
            #     opt_name = new_opt_name
            
            # new opt name method
            new_opt_name = "vafuzz_opt_" + str(idx)
            opt_name_map[new_opt_name] = opt_name
            opt_name_revert_map[opt_name] = new_opt_name
            opt_name = new_opt_name
            
            if "connector" in option:
                connector = option["connector"]
            else:
                connector = "="
            connector_list.append(connector)
            if opt_type == "bool":
                solver_file.write(f"    '{opt_name}': Option('{opt_name}', 'bool', '{prefix}', '{connector}'),\n")
                
            elif opt_type == "intnum":
                range_str = ""
                if "range" in option:
                    low, high = option["range"]
                    range_str = f", range_l={low}, range_h={high}"
                solver_file.write(f"    '{opt_name}': Option('{opt_name}', 'int', '{prefix}', '{connector}'{range_str}),\n")
                
            elif opt_type == "realnum":
                range_str = ""
                if "range" in option:
                    low, high = option["range"]
                    range_str = f", range_l={low}, range_h={high}"
                solver_file.write(f"    '{opt_name}': Option('{opt_name}', 'real', '{prefix}', '{connector}'{range_str}),\n")
                
            elif opt_type == "string":
                solver_file.write(f"    '{opt_name}': Option('{opt_name}', 'string', '{prefix}'),\n")
                
            elif opt_type == "choice":
                choices = option.get("choices", [])
                
                for idx, each_choice in enumerate(choices):
                    # if '-' in each_choice or \
                    #     '.' in each_choice or \
                    #     ':' in each_choice or \
                    #     '+' in each_choice:
                    #     new_each_choice = each_choice.replace("-", "")
                    #     new_each_choice = new_each_choice.replace(".", "")
                    #     new_each_choice = new_each_choice.replace(":", "")
                    #     new_each_choice = new_each_choice.replace("+", "")
                        
                    #     new_each_choice = new_each_choice + str(idx)
                        
                    #     choice_map[new_each_choice] = each_choice
                    #     choice_revert_map[each_choice] = new_each_choice
                    #     choices[idx] = new_each_choice
                    
                    # if each_choice == 'except':
                    #     new_each_choice = 'vafuzz_choice_except'
                    #     choice_map[new_each_choice] = each_choice
                    #     choice_revert_map[each_choice] = new_each_choice
                    #     choices[idx] = new_each_choice
                        
                    # if each_choice.isdigit():
                    #     new_each_choice = f"vafuzz_choice_{each_choice}"
                    #     choice_map[new_each_choice] = each_choice
                    #     choice_revert_map[each_choice] = new_each_choice
                    #     choices[idx] = new_each_choice
                    
                    new_each_choice = f"{opt_name}_choice_{idx}"
                ...
                if choices:
                    ...
                    solver_file.write(f"    '{opt_name}': Option('{opt_name}', 'choice', '{prefix}', '{connector}', choices=[{choices_str}]),\n")
        
        # Close options dictionary
        solver_file.write("}\n\n")
        prefix_list = list(set(prefix_list))
        connector_list = list(set(connector_list))
        solver_file.write(f"prefixes = {prefix_list}\n\n")
        solver_file.write(f"connectors = {connector_list}\n\n")

        # Write the opt name and revert opt name maps
        solver_file.write("opt_name_map = {\n")
        for key, value in opt_name_map.items():
            solver_file.write(f"    '{key}': '{value}',\n")
        solver_file.write("}\n\n")
        
        solver_file.write("opt_name_map_keys = sorted(opt_name_map.keys(), reverse=True)\n\n")
        
        solver_file.write("opt_name_revert_map = {\n")
        for key, value in opt_name_revert_map.items():
            solver_file.write(f"    '{key}': '{value}',\n")
        solver_file.write("}\n\n")

        solver_file.write("opt_name_revert_map_keys = sorted(opt_name_revert_map.keys(), reverse=True)\n\n")

        # Write the choice and reverse choice maps
        solver_file.write("choice_map = {\n")
        for key, value in choice_map.items():
            solver_file.write(f"    '{key}': '{value}',\n")
        solver_file.write("}\n\n")
        
        solver_file.write("choice_map_keys = sorted(choice_map.keys(), reverse=True)\n\n")
        
        solver_file.write("choice_revert_map = {\n")
        for key, value in choice_revert_map.items():
            solver_file.write(f"    '{key}': '{value}',\n")
        solver_file.write("}\n\n")
        
        solver_file.write("choice_revert_map_keys = sorted(choice_revert_map.keys(), reverse=True)\n\n")

        # Building request_config_generation
        header = """
...
def request_config_generation(constraint_file, 
    ...
    solver = Solver()
"""
        solver_file.write(header)
        
        # Add constraints section
        if "conflict" in grammar:
            solver_file.write("    # Conflict constraints\n")
            for conflict_pair in grammar["conflict"]:
                opt1 = clean_opt_name(conflict_pair[0])
                opt2 = clean_opt_name(conflict_pair[1])
                constraint = f"""
    solver.add(Or(Not(options['{opt1}'].z3_var), Not(options['{opt2}'].z3_var)))\n"""
                solver_file.write(constraint)
        
        if "depend" in grammar:
            solver_file.write("   # Dependency constraints: TODO make fixes to non boolean as necessary\n")
            for depend in grammar["depend"]:
                opt = clean_opt_name(depend["opt"])
                dependent_opts = [clean_opt_name(d) for d in depend["on"]]
                # Build the dependency constraint string without using complex f-strings
                dep_vars = [f"options['{d}'].z3_var" for d in dependent_opts]
                dep_vars_str = ", ".join(dep_vars)
                constraint = f"""
    solver.add(Implies(options['{opt}'].z3_var, And({dep_vars_str})))\n"""
                solver_file.write(constraint)



        # Write constraints
        solver_file.write("    # Constraints\n")
...
if __name__ == '__main__':
    main()
"""
        solver_file.write(tail)


def main():
    if len(sys.argv) != 3:
        print("Usage: python generate_solver.py <grammar.json> <output_solver.py>")
        return
    ...
    # Generate the Z3 solver file
    generate_z3_solver(grammar, output_filename)
    print(f"Solver generated: {output_filename}")
