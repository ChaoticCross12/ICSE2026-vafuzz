...
/* Read all testcases from the input directory, then queue them for testing.
   Called at startup. */
// Annotated by ChaticCross12
void read_testcases(afl_state_t *afl, u8 *directory) {

  // nl: array of pointers to dirent. This will store the seed files
  struct dirent **nl;
  ...
  nl_cnt = scandir(dir, &nl, NULL, alphasort); // returns the number of entries

  if (nl_cnt < 0 && directory == NULL) { // check if fails
  ...
  // Old seed selection = vanilla AFL seed selection (random selection)
  // Shuffle only if using vanilla seed selection
  ...
  if (nl_cnt) {
    ...
    do {
      // reduce i by 1 if in_place_resume is set
      if (unlikely(afl->in_place_resume)) { --i; }

      // In GNU/Linux systems, the stat struct is a C structure used to retrieve
      // information about a file or a file system object (like a directory or a
      // symbolic link) stat -> STATUS
      struct stat st;
      // dfn -> directory or file name
      // Here, we store the path of the file with det state done in dfn
      u8          dfn[PATH_MAX];
      snprintf(dfn, PATH_MAX, "%s/.state/deterministic_done/%s", afl->in_dir,
               nl[i]->d_name);

      // Here we store only the path to the file
      u8 *fn2 = alloc_printf("%s/%s", dir, nl[i]->d_name);

      // passed_det -> passed deterministic
      u8 passed_det = 0;
      ...
      /* obviously we want to skip "descending" into . and .. directories,
         ...
         a dot */
      // if we can have sub directories, and current file is a directory,
      // and the first character of the name is not a dot
      if (subdirs && S_ISDIR(st.st_mode) && nl[i]->d_name[0] != '.') {

        free(nl[i]);                                         /* not tracked */
        // recursively call read_testcases with the new sub directory
      ...
      // freeing data from the array of pointers to dirent
      // We will be using fn2 from now on to store the file name
      free(nl[i]);

      // if the file is not a regular file, or the size is 0, or the file name contains
      // "README.txt"
      if (!S_ISREG(st.st_mode) || !st.st_size || strstr(fn2, "/README.txt")) {
      ...
      // if the file size is greater than MAX_FILE.
      // We will be partially reading this file
      if (st.st_size > MAX_FILE) {
      ...
      // MAIN: add the file to the queue
      add_to_queue(afl, fn2, st.st_size >= MAX_FILE ? MAX_FILE : st.st_size,
                   passed_det);

      // if cmplog_mode is set, we will be storing the max file size
  ...
  afl->last_find_time = 0;
  afl->queued_at_start = afl->queued_items;
  OKF("Read a total of %u test cases.", afl->queued_items);
}
...
void perform_dry_run(afl_state_t *afl) {
  afl->execs_counter = 0;
  FILE *f = fopen("/tmp/queue_cycle.log", "w");
  if (f == NULL) { PFATAL("Unable to reset /tmp/queue_cycle.log"); }
  fclose(f);


  struct queue_entry *q; // holder for q
  u32                 cal_failures = 0, idx; // number of failures and index
  u8                 *use_mem;

  // Here we iterate through each of the test cases in the queue
  for (idx = 0; idx < afl->queued_items; idx++) {

    // We store each entry in q
    q = afl->queue_buf[idx];
    // If a q is null or if it is disabled from selection
    if (unlikely(!q || q->disabled)) { continue; }

    u8  res;
    s32 fd;

    // Skipping 0-sized entry in queue
    if (unlikely(!q->len)) {
    ...
    // find the file name and only the file name
    ...
    // allocate memory for the use_mem
...
#undef MSG_FORK_ON_APPLE
  ...
  struct config_queue_entry s;

  // Run z3 test
  // struct dirent *entry;
  // DIR *dp = opendir(afl->pres_conds_dir); // Open the directory
  ...
  //           char full_path[1024];
  //           snprintf(full_path, sizeof(full_path), "%s/%s", afl->pres_conds_dir, entry->d_name);

  //           // Call the function to process the file
  //           run_z3_script(afl, full_path, afl->z3_solver_path);
  //       }
  //   }

    // closedir(dp); // Close the directory
    ACTF("%d", afl->size_of_inject_arr);

  set_temp_config_file_path(afl);
  // variability_driver(afl, true, 1, s);
  // read_config_str_file(afl);
}
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  if (afl->sync_id && mkdir(afl->sync_dir, 0700) && errno != EEXIST) {
    ACTF("Sync ID: %s", afl->sync_id);
    ACTF("Sync Dir: %s", afl->sync_dir);
    ACTF("Sync Dir Error: %d", errno);
    ACTF("%d", mkdir(afl->sync_dir, 0700));
    ACTF("Sync Dir Error EEXIST: %d", EEXIST);
