...
inline u8 va_has_new_bits(u8 *trace_bits, u8 *virgin_map, u32 real_map_size) {
...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  ...
  if (likely(fault == afl->crash_mode)) { // If the fault is a crash or run ok
    ...
    if (likely(classified)) {
      ...
      new_bits = has_new_bits(afl, afl->virgin_bits);// Check if there are new bits
    ...
    if (likely(!new_bits)) { // If the cash does not have new bits, dont save
  ...
  // Case: crash - Will be saved if there are new bits in virgin_bits.
  save_to_queue:
    ...
    afl->id_to_save = afl->queued_items;
#else
    ...
    if (new_bits == 2) { // This means that the new coverage is significant + crashed (maybe)
  ...
  // Here we check:
  // 1. If the fault is a timeout
  //      - If the fault is a timeout, we will save the testcase
  //        if the number of saved timeouts is less than KEEP_UNIQUE_HANG
  // 2. If the fault is a crash
  //      - If the fault is a crash, we will save the testcase
  //        if the number of saved crashes is less than KEEP_UNIQUE_CRASH

  switch (fault) {

    case FSRV_RUN_TMOUT: // 1
...
#ifndef SIMPLE_FILES
// saved in hangs directory
      snprintf(fn, PATH_MAX, "%s/hangs/id:%06llu,%s", afl->out_dir,
               afl->saved_hangs,
               describe_op(afl, 0, NAME_MAX - strlen("id:000000,")));
      snprintf(cfn, PATH_MAX, "%s/hangs_argvs/id:%06llu", afl->out_dir,
               afl->saved_hangs);
#else
    ...
    case FSRV_RUN_CRASH: // Program has crashed, been saved to queue. Now save to crashes
...
#endif                                                    /* ^!SIMPLE_FILES */
      snprintf(cfn, PATH_MAX, "%s/crashes_argvs/id:%06llu", afl->out_dir, afl->saved_crashes);
  ...
  // read config.seed and store the string
  // u8 config_used[500];
  // void *point_to_conf = &config_used;
  // FILE *conf_fd = fopen(afl->temp_config_file_path, "r");
  // if (conf_fd) {
  //   if(fgets(config_used, 100, conf_fd) != NULL) {
  //   } else {
  //     FATAL("Unable to read '%s'", afl->temp_config_file_path);
  //   }
  //   fclose(conf_fd);
  // }
  // strncat(config_used, "\n", 1);
  ...
  for (int i = 0; i < afl->size_of_inject_arr; i++) {
    // Add i to end of cfn
    if (afl->crashed_arr[i] == 1){
      u8  temp[PATH_MAX];
      snprintf(temp, PATH_MAX, "%s_%d", cfn, i);
      // snprintf(cfn, PATH_MAX, "%s", temp);
      fd_conf = fopen(temp, "w");
      if (unlikely(fd < 0)) { PFATAL("Unable to create '%s'", cfn); }
      // ck_write(fd, afl->configs_to_inject_str[i], sizeof(afl->configs_to_inject_str[i]) * afl->size, cfn);
      fprintf(fd_conf, "%s", afl->configs_to_inject_str[i]);
      fclose(fd_conf);
    }
  }  
...
u8 is_interesting(afl_state_t *afl, u8 fault) {
  u8 classified = 0;
  ...
  switch (fault) {
    case FSRV_RUN_CRASH:
      if (!afl->non_instrumented_mode) {
        if (!classified) {
          classify_counts(&afl->fsrv); // Classify trace data
          classified = 1;
        }
        simplify_trace(afl, afl->fsrv.trace_bits); // Simplify trace
        if (!has_new_bits(afl, afl->virgin_crash)) {
          afl->saved_as_crash = false; // Not interesting, clear flag
          return 0;
        }
      }
      afl->saved_as_crash = true; // Mark as crash-worthy
      return 1;

    case FSRV_RUN_TMOUT:
      // Timeouts are not saved as crashes
      afl->saved_as_crash = false;
      return 0;

    case FSRV_RUN_ERROR:
      // Errors are not saved as crashes
      afl->saved_as_crash = false;
      return 0;
    ...
    default:
      if (!afl->non_instrumented_mode) {
        ...
        simplify_trace(afl, afl->fsrv.trace_bits); // Simplify trace
        if (!has_new_bits(afl, afl->virgin_bits)) {
          afl->saved_as_crash = false; // Not a crash, clear flag
          return 0;
        }
      }
      afl->saved_as_crash = false; // Default: not crash-worthy
      return 1;
  }
}
