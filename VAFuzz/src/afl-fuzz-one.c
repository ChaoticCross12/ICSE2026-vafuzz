...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  // FILE *pref_log = fopen("perf_log.txt", "a");
  // fprintf(pref_log, "file: %s, perf_score: %d\n", afl->queue_cur->fname, perf_score);
  // fclose(pref_log);

  // This could be smarter
  // if (perf_score != 0 || perf_score != NULL) {
  //   afl->config_scores[afl->current_iter - 2][afl->config_score_row_counter] = (double)perf_score;
  //   afl->config_score_row_counter++;
    

  //   if (afl->config_score_row_counter == (afl->config_score_expansion_counter * afl->config_score_row_size)) {
  //     afl->config_score_expansion_counter++;
  //     expand_config_scores_matrix(afl);
  //   }
  // }
