...
int main(int argc, char **argv_orig, char **envp) {
  ...
  // still available: ruvz
  while ((opt = getopt(argc, argv,
                       "+a:Ab:B:c:CdDe:E:f:F:g:G:hi:I:Jj:l:L:m:M:nNo:Op:P:QRs:r:q:S:t:"
                       "T:UV:WXx:YZHk:w:K:u:")) > 0) {
    ...
    switch (opt) {
      ...
      case 'w':
        if (afl->pres_conds_dir) { FATAL("Multiple -w options not supported"); }
        afl->pres_conds_dir = optarg;
        break;

      case 'K':
        if (afl->z3_solver_path) { FATAL("Multiple -K options not supported"); }
        afl->z3_solver_path = optarg;
        break;
      ...
      case 'J':
        afl->mutator_1_start = 1;
        break;
      
      case 'r':
        if (afl->grammar_file) { FATAL("Multiple -r options not supported"); }
        afl->grammar_file = optarg;
        break;
      ...
      case 'j':                                              
        ...
        if (afl->pc_rand_selection) { FATAL("Multiple -j options not supported"); }
        if (optarg == NULL) { FATAL("Invalid -j option (got NULL)."); }
        afl->pc_rand_selection = atof(optarg);
        break;

      // MTA
      case 'q':                                                /* Critical value for t-statistic */

        if (afl->execs_per_selection_default) { FATAL("Multiple -q options not supported"); }
        if (optarg == NULL) { FATAL("Invalid -q option (got NULL)."); }
        afl->execs_per_selection_default = atoi(optarg);
        break;

      case 'k':
        if (afl->configs_to_generate) { FATAL("Multiple -k options not supported"); }
        if (optarg == NULL) { FATAL("Invalid -k option (got NULL)."); }
        afl->configs_to_generate = atoi(optarg);
        break;
      
      case 'u':
        if (afl->pc_mute_3_probability) { FATAL("Multiple -u options not supported"); }
        if (optarg == NULL) { FATAL("Invalid -u option (got NULL)."); }
        afl->pc_mute_3_probability = atof(optarg); // 2x of this is mute exhaust prob
        break;
  ...
  get_init_info(afl, afl->z3_solver_path);

  afl->execs_per_selection = afl->execs_per_selection_default;

// Need to allocate memory for this beforehand
  afl->options_hashmap = initHashOptMap();
  
  // Begin: Variability Section
  read_grammar_file(afl);

  if(afl->configuration_file) {
  afl->config_count = count_lines_in_file(afl->configuration_file);
  } else {
    ACTF("No configuration file provided. Generating configs");
  }

  afl->last_config_change = ck_alloc(sizeof(time_t));
  afl->config_score_row_size = 50;

  // if (afl->configuration_file) {
  //   generate_settings_matrix(afl);
  //   generate_config_scores_matrix(afl);
  //   print_config_score(afl);
  //   // read_config_file(afl);
  // } else {
  //   afl->initial_config_count = afl->configs_to_generate;
  //   generate_configurations(NULL, afl, afl->initial_config_count, 0);
  // }
  // afl->current_iter = 1;

  // End: Variability Section
  ...
  while (likely(!afl->stop_soon)) {
    ...
    if (unlikely((!afl->old_seed_selection &&
      ...
      fprintf(f, "Queue cycle %llu:\n", afl->queue_cycle);

      clock_t config_requested = 0, 
              config_received = 0, 
              overall_start = clock(),
              overall_end = 0;
      ...
      if (afl->queue_cycle == 1) {
        ...
        if (afl->mutator_1_start) {
          request_configs(afl, afl->pres_conds_dir, afl->z3_solver_path, 2);
        } else { // havoc
        ...
        // Allocating memory for data triple
      ...
      //   config_requested = clock();
      //   request_configs(afl, afl->pres_conds_dir, afl->z3_solver_path);
      //   config_received = clock();
      //   fprintf(f, "config request time: %f\n", (double)(config_received - config_requested) / CLOCKS_PER_SEC);
      //   fprintf(f, "retrieved configs:\n");
      //   for (u32 i = 0; i < afl->size_of_inject_arr; i++) {
    ...
    do {
      ...
      if (unlikely(afl->old_seed_selection)) {
        ...
        // Check time and decide whether to pause fuzzing and run regerssion
      // time_t cur_time;
      // time(&cur_time);
      // char time_string[80];
      // struct tm *time_info;
      ...
      // if (difftime(cur_time, *afl->last_config_change) > afl->va_driver_wait_time) {
      ...
      //   fclose(f3);
        ...
        // afl->config_scores_lengths[afl->current_iter - 2] = afl->config_score_row_counter;
        // printf("afl->config_score_row_counter: %d\n", afl->config_score_row_counter);
        // selected_pres_cond = variability_driver(afl, false, 1, selected_pres_cond);
      // }
