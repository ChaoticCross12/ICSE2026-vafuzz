...
// Annoted by ChaoticCross12
// Implemented in read_testcases():
// add_to_queue(afl, fn2, some size, passed_det);
...
/* Calculate case desirability score to adjust the length of havoc fuzzing.
   ...
   go into config.h. */
   // Annoted by ChaoticCross12
   // Does not execute anything. Bunch of math
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  // If the total cal cycles is 0, set it to 1
  if (unlikely(!cal_cycles)) { cal_cycles = 1; }
  // If the total bitmap entries is 0, set it to 1
...
// u32 calc_config_score(afl_state_t *afl, struct config_entry *q) { 
