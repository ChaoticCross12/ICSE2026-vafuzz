...
char ** __afl_change_argv(int *argc, char **argv) {
  // argc is uninitilized
  static char inBuf[MAX_CMDLINE_LEN];
  static char *ret[MAX_CMDLINE_PAR];

  char *ptr = inBuf;
  int arg_count = 1;

  FILE * argv_file = fopen(argv[1], "r");
  if (!argv_file) {
    return ret;
  }

  const int bytes_read = fread(inBuf, sizeof(char),
    MAX_CMDLINE_LEN - 2, argv_file);
