...
struct data_triple {
  u32 id;                               /* entry number in queue_buf        */
  bool crashed;                         /* Did the testcase crash?          */
  u8 new_bits;
};


struct config_entry {
    u8 *config_string;                   /* Configuration string   */
    u8 *local_bitmap;                     /* Local bitmap     */
};


struct option_setting {
    u8 *opt;                   /* Option string               */
    // bool is_bool;                             /* Is this a boolean option?      */ 
    bool bool_val;
    // u8 is_choice;                             /* Is this a choice option?      */
    u8 *str_val;                             /* Value string                */
    int int_val;                             /* Value int                */
    double real_val;                             /* Value real */
    u8 *type;                     /* Type string                 */
    bool is_static;
    bool is_negated;
};
...
struct option_entry {
    ...
    u32 id;
    u8 *type;                     /* Type string                 */
    u8 **choices;                 /* Array of choices            */
    int intRangeMin;                 /* Minimum value               */
    int intRangeMax;                 /* Maximum value               */
    double realRangeMin;                 /* Minimum value               */
    double realRangeMax;                 /* Maximum value               */
    u32 strSize;                 /* Maximum string size         */
    bool isChoice;
};
...
typedef struct afl_state {
  ...
  u8 *in_dir,                           /* Input directory with test cases  */
      ...
      *pres_conds_dir,                  /* Working pres conds directory     */
      *z3_solver_path,                   /* Path to target specific z3 solver*/
      *configuration_file,               /* Generated intial command dir    */
      *grammar_file,                    /* Grammar input file               */
...
#define N_FUZZ_SIZE (1 << 21)
  ...
  u32 queued_items,                     /* Total number of queued testcases */
      ...
      config_count,                     /* Number of current configurations */
      config_score_row_size,            /* Size of each row in the config score holder*/
      config_score_expansion_counter,
      current_iter,                    /* Current iteration                */
      config_score_row_counter,
      initial_config_count,             /* Number of initial configurations */
      static_opt_set_count,             /* Number of static option sets     */
      encoded_opt_set_counter,
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  /* Storing program specific grammar object */
  u8 **options_holder;
  u8 **options_type;
  int **options_ranges; // 0 = lower, 1 = upper
  u8 ***options_choices;
  int *options_choices_count;
  u8 **options_connectors;

  /* 2d array of pointers to the options */
  struct option_setting ***option_setting_matrix;

  /* options dictionary*/
  struct HashOptMap *options_hashmap;

  double **config_scores;
  u32 *config_scores_lengths;

  char *temp_config_file_path;

  /* structures for regression analysis */
  double ***encoded_options;

  double *encoded_scores;

  struct HashIntMap *encoded_opt_set_hashmap;
  
  /* Presence condition queue */
  struct option_setting **regression_helper_ls;
  double t_stat_crit_val;

  struct presence_condition_q *presence_condition_queue;

  time_t *last_config_change;

  bool second_run;
  bool first_run;

  u128 va_driver_wait_time;

  u32 configs_to_generate;

  u8 **configs_to_inject_str;
  u8 **z3_configs_to_inject_str;
  u8 **configs_to_inject_local_bitmap;
  int size_of_inject_arr;
  u128 *energy_ls;
  u8 *presence_condition_used;
  int presence_condition_energy_used;

  int **index_to_analyze;
  int *sizes_of_index_to_analyze;
  u8 **variables_to_analyze;
  int size_of_analyze_arr;

  u128 execs_per_selection;
  u128 execs_per_selection_default;
  u128 execs_counter;
  u8 mutator_1_start;
  ...
  struct data_triple **data_triples;

  u8 pc_data_asso;

  double pc_mute_3_probability;
  double pc_rand_selection;

  int *crashed_arr;
  u8 havoc_argv;
  u128 *havoc_argv_scores;

  PyObject *req_config_pfunc;
  PyObject *send_feedback_pfunc;
  PyObject *send_data_feedback_pfunc;
...
u32 calc_config_score(afl_state_t *, struct config_entry *);
...
u8 va_has_new_bits(u8 *trace_bits, u8 *virgin_map, u32 real_map_size);
...
void  generate_configuration_seed(afl_state_t *);
